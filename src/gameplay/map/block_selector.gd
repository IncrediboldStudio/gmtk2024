...
@export var conveyor_block_data : BlockData
...
func get_block_for_tile(selected_tile : SelectTile):
    if currently_selected_block == conveyor_block_data:
        if last_tile_dragged and last_tile_dragged.placed_block:
            ...
            return get_conveyor_block_data_variant(input_direction, output_direction)
        return conveyor_block_data
    return get_block()
...
func update_last_placed_tile(selected_tile :  SelectTile):
    if last_tile_dragged.placed_block and _is_conveyor_belt(last_tile_dragged.placed_block.block_data):
        ...
        last_tile_dragged.placed_block.block_data = get_conveyor_block_data_variant(input_direction, output_direction)
        
func get_conveyor_block_data_variant(input : BlockIO.Direction, output : BlockIO.Direction):
