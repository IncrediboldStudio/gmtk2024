...
func _place_block(selected_tile : SelectTile):
    if _is_block_placable(selected_tile.map_pos):
        ...
        if block_selector.last_tile_dragged:
            if block_selector._is_conveyor_belt(selected_tile.placed_block.block_data):
                var input_direction = block_selector._get_direction_between_tiles(block_selector.last_tile_dragged, selected_tile)
                var output_direction = input_direction
                selected_tile.placed_block.block_data =  block_selector._get_conveyor_block_data_variant(input_direction, output_direction)
